...
- Segundo intento: `padding-left: 88px; margin-left: -88px;` (24px en mobile) en `.pcd-docentes__grid`, espejo del bleed que ya usaba el lado derecho. Matemáticamente el ancho de contenido del grid queda igual (padding y margin se siguen cancelando), pero en la práctica **rompió por completo el layout del carrusel**: las cards perdían su ancho (`flex: 0 0 calc(...)`), el texto del bio dejaba de hacer wrap y se salía de pantalla, y el borde/fondo de la card desaparecía — confirmado por el usuario incluso después de hard refresh y reiniciar `npm run dev`, así que no era caché del dev server. Sospecha: navegadores tienen bugs conocidos con márgenes negativos grandes en cálculos de ancho intrínseco (shrink-to-fit) en algún ancestro de la cadena de layout: con -8px nunca se notaba, con -88px sí.
- **Revertido por completo** a `padding-left: 8px; margin-left: -8px;` (idéntico byte a byte al commit `d0f9bce`, confirmado con `git diff`). El pedido original de "que no se corte en la margen" queda sin resolver por ahora — se prioriza que el carrusel vuelva a verse bien.
- Archivos: `src/styles/programa.css`.
...
- CSS override final `.pcd-docente--nicolas`: `scale(1.1)` en la card, **igual en default y hover** (sin el achique 0.94 del resto de docentes) — para que la animación init→end no se note un salto de tamaño.
- Mismo criterio en el modal: `.pcd-docente-modal--nicolas` fija `scale(1.1)` en ambas capas (`--init` y `--end`), en vez del 1.08 / 1.08*0.94 por defecto.
